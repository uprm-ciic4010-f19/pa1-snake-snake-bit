...
public class Player {
	...
	public int isAppleGoodCounter; // Counter for Apple turning bad. -Jesus
	public boolean isAppleGood = true; // Determines Apple being good. -Jesus
	...
	public void tick(){
		...
		if(moveCounter>=moveSpeedChange) {
			...
			moveCounter=0;
			isAppleGoodCounter++; // For turning Apple bad. -Jesus
			if (isAppleGoodCounter % 80 == 0) {
				isAppleGood = false;
	...
	public void checkCollisionAndMove(){
		...
		// Based on whether apple is good or rotten, the action taken. -Jesus
		if (isAppleGood) {
			...
			if(handler.getWorld().appleLocation[xCoord][yCoord]){
				...
				if (scoreNum != 0) { 
					scoreNum -= Math.sqrt(2 * scoreNum + 1);
				}
				isAppleGood = true;
		...
		// When it collides with itself, it'll trigger kill() -Jesus
	...
	public void render(Graphics g,Boolean[][] playeLocation){
		...
		for (int i = 0; i < handler.getWorld().GridWidthHeightPixelCount; i++) {
			for (int j = 0; j < handler.getWorld().GridWidthHeightPixelCount; j++) {
				...
				if (isAppleGood) { //When apple is good, apple is colored red. -Jesus
				...
				} else { //When apple is rotten, apple is colored black. -Jesus
	...
	public void Eat() {
		...
		isAppleGoodCounter = 0; // Resets counter for Apple turning bad, as if it's a new, good apple. -Jesus
	...
	public void kill(){
		...
		// Game Over prompt after snake triggers kill(), then it goes back to the title screen. -Jesus
